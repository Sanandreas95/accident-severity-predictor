...
# Editing for precision Fixing (15 july)


# # Test different thresholds
# print("\nTesting thresholds...")
# for threshold in [0.5, 0.6, 0.7, 0.8]:
#     y_pred = (y_test_pred_proba_xgb > threshold).astype(int)
#     precision = precision_score(y_test, y_pred)
#     recall = recall_score(y_test, y_pred)
#     f1 = f1_score(y_test, y_pred)
#     print(f"Threshold {threshold}: Precision={precision:.2%}, Recall={recall:.2%}, F1={f1:.2%}")




# IMPROVED: Test different thresholds for BALANCED results

print("\n" + "="*120)
print("TESTING DIFFERENT DECISION THRESHOLDS - Find Balance Between Precision & Recall")
print("="*120)
print(f"{'Threshold':<12} {'Precision':<15} {'Recall':<15} {'F1-Score':<15} {'Accuracy':<15}")
print("-"*120)
...
# Test MORE thresholds with smaller steps for better granularity
thresholds_to_test = [0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]

for threshold in thresholds_to_test:
    y_pred_threshold = (y_test_pred_proba_xgb > threshold).astype(int)
    ...
    # Mark GOOD BALANCE thresholds: Precision > 45% AND Recall > 65%
    is_balanced = precision > 0.45 and recall > 0.65
    marker = " ← GOOD BALANCE! ✓" if is_balanced else ""
    
    print(f"{threshold:<12.2f} {precision:<15.2%} {recall:<15.2%} {f1:<15.2%} {accuracy:<15.2%}{marker}")
...
print(f"   Precision: {threshold_results[best_threshold]['precision']:.2%}")
print(f"   Recall: {threshold_results[best_threshold]['recall']:.2%}")
...
y_pred_xgb_final = (y_test_pred_proba_xgb > OPTIMAL_THRESHOLD).astype(int)
y_pred_proba_xgb_final = y_test_pred_proba_xgb
...
print("\n" + "="*120)
print(f"FINAL MODEL PERFORMANCE (Using Optimal Threshold: {OPTIMAL_THRESHOLD})")
print("="*120)

# Recalculate metrics with optimal threshold
final_accuracy = accuracy_score(y_test, y_pred_xgb_final)
final_precision = precision_score(y_test, y_pred_xgb_final)
final_recall = recall_score(y_test, y_pred_xgb_final)
final_f1 = f1_score(y_test, y_pred_xgb_final)
final_roc_auc = roc_auc_score(y_test, y_pred_proba_xgb_final)

print(f"\nAccuracy:  {final_accuracy:.2%}  (was 76.24%)")
print(f"Precision: {final_precision:.2%}  (was 38.64%) → Improvement: {(final_precision-0.3864)*100:.2f}%")
print(f"Recall:    {final_recall:.2%}  (was 75.77%)")
print(f"F1-Score:  {final_f1:.2%}  (was 51.18%) → Improvement: {(final_f1-0.5118)*100:.2f}%")
print(f"ROC-AUC:   {final_roc_auc:.2%}  (was 83.44%)")
print("="*120 + "\n")
...
# Use metrics from optimal threshold
xgb_precision = final_precision
xgb_recall = final_recall
xgb_f1 = final_f1
xgb_roc_auc = final_roc_auc
